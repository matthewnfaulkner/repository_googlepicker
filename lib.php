 ...
 */
class repository_googlepicker extends repository_googledocs {
    ...
    public function __construct($repositoryid, $context = SYSCONTEXTID, $options = array(), $readonly = 0) {
        ...
        try {
            $this->issuer = \core\oauth2\api::get_issuer(get_config('googlepicker', 'issuerid'));
    ...
    public function supported_returntypes() {
        ...
        if (!empty($this->issuer) && $this->issuer->is_system_account_connected()) {
            $setting = get_config('googlepicker', 'supportedreturntypes');
            if ($setting == 'internal') {
    ...
    public function default_returntype() {
        $setting = get_config('googlepicker', 'defaultreturntype');
        $supported = get_config('googlepicker', 'supportedreturntypes');
    ...
    public function reference_file_selected($reference, $context, $component, $filearea, $itemid) {
        ...
        $cache = cache::make('repository_googlepicker', 'folder');
        $parentid = 'root';
        $fullpath = 'root';
        $allfolders = [];
    ...
    public function get_reference_details($reference, $filestatus = 0) {
        ...
        if ($owner) {
            return get_string('owner', 'repository_googlepicker', $owner);
        } else {
            return $info->name;
        }
    }
    ...
    public static function type_config_form($mform, $classname = 'repository') {
        ...
        $mform->addElement('static', null, '', get_string('fileoptions', 'repository_googlepicker'));
        $choices = [
            'internal' => get_string('internal', 'repository_googlepicker'),
            'external' => get_string('external', 'repository_googlepicker'),
            'both' => get_string('both', 'repository_googlepicker')
        ];
        $mform->addElement('select', 'supportedreturntypes', get_string('supportedreturntypes', 'repository_googlepicker'), $choices);

        $choices = [
            FILE_INTERNAL => get_string('internal', 'repository_googlepicker'),
            FILE_CONTROLLED_LINK => get_string('external', 'repository_googlepicker'),
        ];
        $mform->addElement('select', 'defaultreturntype', get_string('defaultreturntype', 'repository_googlepicker'), $choices);

        $mform->addElement('static', null, '', get_string('importformat', 'repository_googlepicker'));
        ...
        $mform->addElement('select', 'documentformat', get_string('docsformat', 'repository_googlepicker'), $docsformat);
        ...
        $mform->addElement('select', 'drawingformat', get_string('drawingformat', 'repository_googlepicker'), $drawingformat);
        ...
        $str = get_string('presentationformat', 'repository_googlepicker');
        ...
        $str = get_string('spreadsheetformat', 'repository_googlepicker');
